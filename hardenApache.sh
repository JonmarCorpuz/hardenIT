...
# ==== STATIC VARIABLES =====================================================================
WHITE="\033[0m"           # Réinitialise la couleur (blanc/neutre par défaut)
RED="\033[0;31m"          # Définit la couleur rouge (utilisé pour les erreurs)
YELLOW="\033[0;33m"       # Définit la couleur jaune (utilisé pour les avertissements)
GREEN="\033[0;32m"        # Définit la couleur verte (utilisé pour les succès ou validations)
GREY="\033[38;5;254m"     # Définit une couleur gris clair (souvent pour les textes neutres)


# ==== FUNCTIONS ============================================================================
usageError() {                                                                           # Fonction pour afficher un message d'erreur d'utilisation
  echo -e "\n${RED}[ERROR]${WHITE} Usage: ./hardenUbuntu.sh [--commit|--difference]\n"   # Affiche un message d'erreur avec les options d'utilisation valides du script
  exit 1                                                                                 # Termine l'exécution du script avec un code d'erreur (1)
}
...
# ==== REQUIREMENTS =========================================================================
setVariables                                     # Charge les variables depuis les fichiers de configuration
sudo chmod +x ./Scripts/*                        # Rend tous les scripts du dossier Scripts exécutables

# Sauvegardes
sudo cp /etc/apache2/apache2.conf "/etc/apache2/apache2.conf.bak.$(date +%F-%T)"                                  # Sauvegarde du fichier apache2.conf avec horodatage
sudo cp /etc/apache2/conf-available/security.conf "/etc/apache2/conf-available/security.conf.bak.$(date +%F-%T)"  # Sauvegarde du fichier security.conf avec horodatage


# ==== MAIN BODY ============================================================================

if [[ $SERVERTOKENS_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening ServerTokens"

  # PARAMETER 1: Ensure ServerTokens is set to Prod or ProductOnly
...
if [[ $ROOTDIRECTORY_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening Apache web server's root directory"

  # PARAMETER 2: Ensure Options for the OS Root Directory are restricted
  ...
  # PARAMETER 3: Ensure Access to OS Root Directory is denied by default
  if ls -ali /etc/apache2/apache2.conf &> /dev/null;
  then 
  ...
  # PARAMETER 4: Ensure OverRide is disabled for the OS Root Directory
  if ls -ali /etc/apache2/apache2.conf &> /dev/null;
  then 
...
if [[ $WEBROOTDIRECTORY_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening Apache web server's web root directory"

  # PARAMETER 5: Ensure Options for the Web Root Directory are restricted
  if ls -ali /etc/apache2/apache2.conf &> /dev/null;
  then 
...
if [[ $APACHEUSER_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening"

  # PARAMETER 6: Ensure the Apache web server runs as a non-root user
  if ls -ali /etc/apache2/apache2.conf &> /dev/null;
  then 
  ...
  # PARAMETER 7: Ensure the Apache user account has an invalid shell
  ...
  # PARAMETER 8: Ensure the Apache user account is locked
  ...
  # PARAMETER 9: Ensure Apache directories and files are owned by root
  ...
  # PARAMETER 10: Ensure the group is set correctly on Apache directories and files
  ...
  # PARAMETER 11: Ensure other write access on Apache directories and files are restricted
...
if [[ $COREDUMP_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening"

  # PARAMETER 12: Ensure the Core Dump directory is secured
  ...
  # PARAMETER 13: Ensure group write access for the Apache directories and files is properly restricted
  ...
  # PARAMETER 14: Ensure group write access for the document root directories and files is properly restricted
...
if [[ $OPERATIONS_HARDENING =~ true ]];
then
  echo -e "\n${GREY}[NOTICE]${WHITE} Hardening"

  # PARAMETER 15: Ensure the error log filename and severity are configured correctly
  if grep -Ri '^\s*LogLevel' /etc/apache2/apache2.conf &> /dev/null;
  then

    if grep -Ri '^\s*LogLevel' /etc/apache2/apache2.conf | grep "info" &> /dev/null;
    then 
      echo -e "${GREEN}[PASS]${WHITE} LogLevel configurations conforms to the CIS Benchmark"
    else 
      echo -e "${RED}[FAIL]${WHITE} LogLevel configurations don't conform to the CIS Benchmark"
